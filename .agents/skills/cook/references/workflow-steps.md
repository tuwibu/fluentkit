...
**Task Tool Fallback:** `update_plan`/`update_plan`/`update_plan`/`update_plan` are CLI-only — unavailable in VSCode extension. If they error, use `update_plan` checklist for progress tracking. All workflow steps remain functional without Codex subagent workflows.
...
**Parallel mode:**
- Utilize all tools of Codex task plan items: `update_plan`, `update_plan`, `update_plan` and `update_plan`
...
1. **MUST** activate `/project-management` skill (MANDATORY) — run full sync-back for [plan-path]: reconcile all completed Codex task plan items with all phase files, backfill stale completed checkboxes across every phase, then update plan.md frontmatter/table progress. Do NOT only mark current phase.
...
4. Use `update_plan` to mark Codex task plan items complete after sync-back confirmation.
...
- **MANDATORY DELEGATION:** Steps 4, 5, 6 MUST delegate via Codex subagent workflow / skill activation. DO NOT implement directly.
  - Step 4: `tester` (and `debugger` if failures)
  - Step 5: `code-reviewer`
  - Step 6: `/project-management` skill, `docs-manager`, `git-manager`
- Use `update_plan` to create Codex task plan items for each unchecked item with priority order and dependencies (or `update_plan` checklist if Codex subagent workflows unavailable).
- Use `update_plan` to mark Codex task plan items `in_progress` when picking up a task (skip if Codex subagent workflows unavailable).
- Use `update_plan` to mark Codex task plan items `complete` immediately after finalizing the task (skip if Codex subagent workflows unavailable).
- All step outputs follow format: `✓ Step [N]: [status] - [metrics]`
- **VALIDATION:** If Codex subagent spawns = 0 at end of workflow, the workflow is INCOMPLETE.
