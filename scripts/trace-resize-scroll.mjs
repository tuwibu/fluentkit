import { chromium } from 'playwright'
const b = await chromium.launch({ headless: true })
const p = await (await b.newContext({ viewport:{width:1100,height:800} })).newPage()
await p.goto('http://localhost:5173/profiles', { waitUntil:'networkidle' }); await p.waitForTimeout(900)
function measure(){ return p.evaluate(()=>{
  const tbl=document.querySelector('[data-slot="data-table"] table')
  const scroll=tbl.closest('.overflow-auto')||tbl.parentElement
  return { tableW: Math.round(tbl.getBoundingClientRect().width), tableSW: tbl.scrollWidth,
    scrollClientW: scroll.clientWidth, scrollSW: scroll.scrollWidth, canScrollX: scroll.scrollWidth>scroll.clientWidth,
    tableCls: tbl.className.split(' ').filter(c=>/^w-|min-w|table-/.test(c)).join(' ') }
})}
const before = await measure()
// drag Name handle far right (+600)
const th = p.locator('th:has-text("Name")').first()
const h = th.locator('[data-slot="column-resize-handle"]'); const box = await h.boundingBox()
await p.mouse.move(box.x+3, box.y+box.height/2); await p.mouse.down()
await p.mouse.move(box.x+600, box.y+box.height/2, { steps: 15 }); await p.mouse.up()
await p.waitForTimeout(400)
const after = await measure()
console.log(JSON.stringify({ before, after }, null, 1))
await b.close()
