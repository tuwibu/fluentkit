/**
 * E2E tests for decision-visibility-gate.cjs hook entry (spawned via child_process)
 * Run: node .claude/hooks/__tests__/decision-visibility-gate.test.cjs
 *
 * Scenarios: fail-open on missing transcript / malformed stdin, config disable,
 * default-enabled blocking, minimal-mode override.
 *
 ...
 * Config location: <cwd>/.claude/.ck.json → hooks['decision-visibility-gate']
 *
 * Exit codes:
 *   0 → allowed / fail-open
...
/**
 * Create a temp project dir with .claude/.ck.json.
