/**
 * Tests for getHookOption() — new export of ck-config-utils.cjs (TDD red step)
 * Run: node .claude/hooks/lib/__tests__/get-hook-option.test.cjs
 *
 * getHookOption(hookName, key, defaultValue):
 *   Returns config.hooks[hookName][key] when the hook value is a plain object;
 *   otherwise returns defaultValue.
 *
 * Config is read from claude/.ck.json relative to process.cwd().
 * Tests create a temp dir + write a minimal claude/.ck.json, then chdir into it.
 */
...
/**
 * Create a temp project dir with claude/.ck.json containing hooksConfig,
...
function withConfig(hooksConfig, fn) {
  ...
  fs.writeFileSync(path.join(claudeDir, '.ck.json'), JSON.stringify(config), 'utf8');
...
/** Create temp dir with NO claude/.ck.json at all. */
...
test('hooks section absent entirely → returns defaultValue', () => {
  // Write a config with no hooks key at all
  const tmpDir = path.join(os.tmpdir(), 'ck-test-no-hooks-' + Date.now());
  const claudeDir = path.join(tmpDir, 'claude');
  fs.mkdirSync(claudeDir, { recursive: true });
  fs.writeFileSync(path.join(claudeDir, '.ck.json'), JSON.stringify({ workflow: {} }), 'utf8');
  process.chdir(tmpDir);
  try {
...
test('no config file → returns defaultValue, no throw', () => {
  withNoConfig(() => {
    // This test verifies that once getHookOption is implemented,
    // it gracefully returns the default when no config file exists.
    // Currently fails with "not a function" — will pass after implementation.
...
test('isHookEnabled still returns true when hook value is object (backward compat)', () => {
  // isHookEnabled uses `!== false` — object is truthy, must still return true
