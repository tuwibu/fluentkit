...
/** Get files touched in recent commits (fallback when working tree is clean after commit) */
function recentCommitFiles(cwd) {
  try {
    const out = execGit(['log', '--name-only', '--pretty=format:', '-n', '3'], cwd);
    if (!out) return [];
...
function extractSessionData(stdinData) {
  ...
  // Modified files: union of working-tree diff + recent commits (dedup, cap 20)
  const cwd = stdinData.cwd || process.cwd();
  const gitDiff = (() => {
    try {
      const diff = execGit(['diff', '--name-only', 'HEAD'], cwd);
      return diff ? diff.split('\n').filter(Boolean) : [];
    } catch { return []; }
  })();
  const recent = recentCommitFiles(cwd);
  const seen = new Set();
  const merged = [];
  for (const f of [...gitDiff, ...recent]) {
    if (f && !seen.has(f)) { seen.add(f); merged.push(f); }
  }
  data.modifiedFiles = merged.slice(0, 20);
  return data;
}
...
function buildStateContent(data) {
  ...
  let workedLines;
  if (completed.length) {
    workedLines = completed.map(t => `- ${t.content}`);
  } else if (data.modifiedFiles && data.modifiedFiles.length > 0) {
    workedLines = [`- Đã chạm: ${data.modifiedFiles.slice(0, 5).join(', ')}`];
  } else {
    workedLines = ['- (No completed tasks recorded)'];
  }
  const lines = [
    '# Session State',
...
/** Extract last assistant text block from transcript JSONL (sync, best-effort) */
function extractLastAgentText(transcriptPath) {
  try {
    ...
    // Strip markdown noise and cap at ~150 chars
    const clean = lastText.replace(/\n+/g, ' ').replace(/\s{2,}/g, ' ');
...
function buildAgentSection(stdinData) {
  ...
  const summary = extractLastAgentText(stdinData.transcript_path);
  const body = summary ? `- ${summary}` : `- Completed at ${ts}`;
