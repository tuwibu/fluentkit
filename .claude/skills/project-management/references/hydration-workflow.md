...
│ + TodoWrite      │  ← Optional live cursor for session-scoped substeps
└──────────────────┘
```
...
3. (Optional) Mirror current-phase items into `TodoWrite` for live agent visibility — only when phase has 3+ substeps worth tracking.
...
- Pick up an item → set TodoWrite status `in_progress` (optional).
...
**Plan files survive everything:** session restart, branch switch, machine swap, even Claude Code reinstall.
