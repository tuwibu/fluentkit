...
| Session-scoped subtasks | `TodoWrite` tool | Ephemeral (clears per session) |
| Format validation | `plan-format-kanban.cjs` hook | Auto PostToolUse |

**Why this design:** Plan files survive session restart, branch switch, machine swap. TodoWrite covers in-session granular tracking only.
...
- Optionally use `TodoWrite` to mirror in-session unchecked items for live tracking
...
- **Track in-session:** Optional — `TodoWrite` mirror for live agent visibility
...
3. As work progresses: Edit `[ ]` → `[x]` directly in phase files; optionally TodoWrite for session-scoped subtasks
