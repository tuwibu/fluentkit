...
| `--preview` | Set `preview: true` in `plan.md` frontmatter → embed/refresh an inline mermaid business-flow diagram at the pre-cook gate (see `.claude/rules/business-flow-diagram.md`) |
...
- **Business-flow diagram (`--preview` only):** per `.claude/rules/business-flow-diagram.md` — embed an inline mermaid flow in `plan.md` ONLY when invoked with the `--preview` flag; otherwise draw nothing.
