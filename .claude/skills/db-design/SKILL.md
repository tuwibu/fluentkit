...
Invoked when a schema change adds a column/field that existing rows/documents must be populated with. The single non-negotiable rule mirrors Step 5: **schema DDL first, always via ORM migrate — never hand-write migration SQL**. Only after the migration is applied do you address existing data.

### Decision tree

```
Is this a dev / local environment?
├── YES → recommend migrate reset + reseed (see Dev path below). No backfill script needed.
└── NO  → generate a TypeScript backfill script for the user to review. Never auto-apply.
```

### Dev path — reset + reseed (preferred)

In dev there is no real data worth preserving. The simplest, safest fix:

```bash
# Prisma
npx prisma migrate reset        # drops DB, replays migrations, runs seed
npx prisma db seed

# TypeORM (no built-in reset — equivalent)
npx typeorm migration:revert -d src/data-source.ts   # revert to before the change
npx typeorm migration:run -d src/data-source.ts      # re-apply with new column
# then seed manually

# Drizzle
npx drizzle-kit drop && npx drizzle-kit migrate
# then seed manually
```

Instruct the user to run the reset themselves — the `db-readonly-guard` hook blocks destructive ops from this skill (see DESTRUCTIVE OPS POLICY above).

### Prod path — generate a TypeScript backfill script

When backfill is genuinely required (production, shared staging, or any env with real data):

1. **Generate** a TS script the user must review and run manually — never auto-execute.
2. Use the template in `references/backfill-script-template.md` as the skeleton.
3. Populate the skeleton with:
   - The correct ORM client import for the detected stack.
   - The target model/table and the new column/field name.
   - Any existing domain validators, services, or types to reuse (the main reason TS beats SQL here — shared business logic, type safety, no duplication).
   - The correct `where` condition for idempotency (rows where the new field is still null/unset).
4. Save the generated script to `scripts/backfill/YYMMDD-<name>.ts` and print the path.
5. Remind the user:
   - Run `DRY_RUN=true` first — logs what would change, touches nothing.
   - Review the output, then run without the flag.
   - The script is idempotent — safe to re-run.
   - **Never pipe this script directly into prod CI/CD without a manual review gate.**

### What the generated script MUST contain

| Requirement | Why |
|---|---|
| `DRY_RUN` env flag — default `true` | Forces the user to opt in to writing; prevents accidental prod mutations |
| Batch loop (`skip` / `take` or cursor-based) | Avoids OOM on large tables; keeps transaction size predictable |
| Idempotent `where` filter (`newField: null`) | Safe to re-run after partial failure; no double-processing |
| Progress logging per batch | Visibility; the user knows it's running and can estimate completion |
| Import of existing domain logic/validators | DRY — reuses the source of truth; detects business-rule violations early |
| No raw SQL | TS + ORM keeps type safety and reuses migration history; raw SQL can diverge silently |

### Anti-patterns (backfill-specific)

| Smell | Why it's bad |
|---|---|
| Writing `UPDATE table SET col = value WHERE col IS NULL` in a migration file | Mixes DDL and DML — migration runners retry on failure, double-applying DML mutates data twice |
| Bulk `updateMany` without batching | Single query on a 10M-row table → OOM, lock escalation, timeout |
| Skipping dry-run | First run on prod with live writes = no recovery path if logic is wrong |
| Hardcoding business logic in the backfill | When the rule changes, the backfill diverges silently — import the service/validator instead |
| Auto-applying the script from this skill | Backfill output is review-only; the human decides when prod data moves |
