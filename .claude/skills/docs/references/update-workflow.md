...
## Additional requests
<!-- $ARGUMENTS substituted here excludes the --preview token, which is handled by Phase 2.5 -->
<additional_requests>
  $ARGUMENTS
</additional_requests>

## Phase 2.5: Business Flows (--preview only)

**Skip this phase entirely if `--preview` was not passed.**

Load and follow `references/flows-workflow.md` (update mode). That file is self-contained:
staleness check per existing flow file → regenerate if sources changed → create missing domains
→ warn (do not delete) on removed source paths.

## Phase 3: Size Check (Post-Update)

After docs-manager (and Phase 2.5, if run) completes:
